class MeetingDoodle < ActiveRecord::Base
  ...
  has_many :comments,  -> { order( "#{Comment.table_name}.created_on ASC") }, :as => :commented, :dependent => :delete_all
  has_many :responses,  -> { order(  "#{MeetingDoodleAnswer.table_name}.updated_on ASC") }, :class_name => 'MeetingDoodleAnswer', :dependent => :destroy
  acts_as_watchable
